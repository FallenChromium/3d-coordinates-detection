...
def process_cameras(sheet_data: DataFrame):
    ...
    for cam_number, cam_sheet_row in enumerate(cameras_indicies):
        ...
        camera = f"camera_{cam_number}"
        cameras[camera] = {
            "position": {
                "x_m": sheet_data.iat[cam_sheet_index["row"] + x_shift[0], cam_sheet_index["column"] + x_shift[1]],
                "y_m": sheet_data.iat[cam_sheet_index["row"] + y_shift[0], cam_sheet_index["column"] + y_shift[1]],
                "z_m": sheet_data.iat[cam_sheet_index["row"] + z_shift[0], cam_sheet_index["column"] + z_shift[1]],
                "azimuth_deg": sheet_data.iat[cam_sheet_index["row"] + azimuth_shift[0], cam_sheet_index["column"] + azimuth_shift[1]],
            },
            "matrix": {
